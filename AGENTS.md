...
| Did I add a path to the OpenAPI spec? Is a handler mounted? | About half the `/v1/admin/*` paths in `api/openapi.yaml` are documented-but-unserved; `docs/architecture/admin-control-surface.md` is the served/planned split. A spec entry with no handler is a lie in the contract: the 2026-08-09 round close found `/v1/admin/cost-events` declared as served with nothing behind it and graded it *"contract claiming a nonexistent endpoint — **worst**"* (`../docs/90_archive/reports/ROUND-CLOSING-REPORT-2026-08-09.md` §4). Nothing gates this. |
...
| Do my tests fail if I revert the fix? | Nothing forces the revert. Across this workspace 40 of 70 mutation probes survived a fully green run (`../docs/90_archive/reports/QA-SPAN-2026-08-11.md`). A guard you have not watched go red is not a guard. |
