...
| [`ADR-I003`](ADR-I003-cost-optimization-strategy.md) | **Proposed** — the only one not accepted; **three of its premises are corrected by `ADR-I004`** | Anchor amortization as the structural cost model ("derive, don't regenerate") plus deferred levers. Carries a provenance caveat: its cost figures are **provisional**; the decided content is the structural model and the deferral triggers, not the numbers. Renamed from `ADR-P003`. |
| [`ADR-I004`](ADR-I004-measured-cost-optimization-findings.md) | accepted (2026-08-31) | What the platform actually spends, measured against the code. Bounds billable provider calls to the units a reservation priced; quantizes the reuse key to the precision the column stores (`gv=5`); makes draft→commit reuse one-way; makes the fail-open budget observable and seeds a dev cap. **Corrects three premises of `ADR-I003`** (uncapped spend by default; the four-tier ladder is a no-op for `/v1/generations`; draft ranking already ships and buys nothing) and replaces two unfalsifiable deferral triggers. |
| [`ADR-I005`](ADR-I005-second-scene-route-and-terminal-refusals.md) | accepted (2026-09-01) | Written after a total art blackout: 544 assets sat `ready` while every picture was unfetchable. `scene_capable` had one real route (bfl), because Kontext fails closed without a reference, so one account's credit was a whole-product outage. Adds `provider_unpaid` as a terminal-but-**walkable** refusal, `fal_t2i` (prompt-only scenes, priority 150), sends the permissiveness dial on every provider that has one, and derives resolver availability from the **registry** — a hand-written second list had made `fal_dev`'s and `fal_t2i`'s routes reconcile `valid` and be silently unselectable. |
