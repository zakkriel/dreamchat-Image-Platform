...
### Entry 106
- **Trigger**: After fixing the service-side persistence and adding capability-aware routing, CI's migrations job still failed — but now at a *different* test (`TestEndToEndArtifactReuseMisses`, an S3-gated test that `t.Skip`s locally without MinIO), with `422 no_route` on a `quality_tier: "high"` artifact request
- **Category**: surprise (an existing test exercised a quality tier the seed never had a route for, and I couldn't run it locally)
- **Note**: `quality_tier` is a HARD filter in route resolution (the reviewer frames capability as "routes exist for operation/quality but none satisfy capability", treating quality as a prior filter — and the resolver unit test pins exactly that). But the only seeded mock route (0002) is `standard`, while the handler validates and forwards `draft`/`standard`/`high`. Pre-7A this didn't matter — there was no resolver, so quality never gated routing; all qualities generated via the hardcoded mock. Phase 7A made quality gate, so a `high` request resolved nothing. Fix: seed `draft` and `high` `scene_capable` mock routes in 0006 (the mock model renders any tier; price is keyed on provider+model+operation, not quality, so the single-active-price index is untouched). I could NOT reproduce locally — the test is S3-gated and MinIO won't install (download 403'd) — but the failure is at route resolution in the *handler*, strictly before any S3 work, so I verified the exact fix with a resolver DB integration test asserting all three scene quality tiers resolve to their routes. Lesson: when a filter goes from "unused" to "enforced", every value the API accepts for that filter needs a seeded row — and S3-gating hides a whole class of these from the local run, so grep every integration test for the filter's values rather than trusting the local suite's green.

### Entry 107
- **Trigger**: Phase 7B's "the preview_ready state must be real and externally observable" + "preview persistence and final persistence must be separate DB transactions" collided with the single-invocation provider model the same task mandates ("worker may complete preview and final in one worker task invocation")
- **Category**: surprise (an observability requirement on an intermediate state that the worker passes through in a few milliseconds within one synchronous task)
- **Note**: The natural worker shape is one function that calls `Generate` twice and writes rows; the temptation is to wrap it in one transaction for atomicity. But the spec's hard requirement is the opposite: the preview asset insert AND the `preview_ready` job flip must **commit before** final generation begins, so another process can read the preview through `GET /v1/jobs/{id}` and `/assets` before the final exists. The worker already uses auto-commit single statements (no explicit tx around the single-phase insert + MarkCompleted), so I leaned into that: `processPreviewFirst` does `InsertPreview` (commit) → `MarkPreviewReady` (commit) → *then* `Generate(final)`. No shared transaction is even possible across the two phases because each repo call is its own statement. Proving observability was the harder part — a single-invocation worker passes through `preview_ready` too fast to catch over HTTP. Two complementary proofs: (1) a **unit** provider that, on its *final* call, reads the job back and asserts it already shows `preview_ready` + one `preview_asset_id`; (2) an **integration** test that runs the worker in a goroutine with a provider that **blocks** on the final `Generate`, then issues real `GET /v1/jobs/{id}` and `/assets` and sees the preview before releasing final. Lesson: when "intermediate state must be observable" meets "do it all in one task," don't fake the state — commit it for real between phases and prove observability by *pausing* the worker at the phase boundary, not by hoping a fast pass-through is catchable.

### Entry 108
- **Trigger**: The worker has to decide "is this a two-phase job?" but `generation_jobs` has no provider/route columns and the worker is forbidden to re-resolve — and the rule needs **both** `delivery_mode==preview_first` **and** the resolved route's `preview_capability==true_preview`, the second of which lived only in the resolver's output, not on the job
- **Category**: process (a worker decision needs a route attribute that was never persisted)
- **Note**: Phase 7A persists `provider_id`/`model_id`/`provider_route_id` on `input_payload` but not `preview_capability`. The resolver guarantees a `preview_first` request only ever resolves a `true_preview` route (else 422), so in principle `delivery_mode==preview_first` alone implies true_preview. But the task explicitly wants the worker to verify **both**, as a belt-and-suspenders guard against a malformed payload silently two-phasing. Rather than have the worker re-resolve (forbidden) or look the route up in the DB (a new query + a re-resolution in disguise), I extended `applyResolvedRoute` to also stamp `preview_capability` onto the payload — it's pure route provenance, exactly like the provider/model/route already stamped there, and harmless for final-only/pack jobs (the worker only two-phases when `delivery_mode==preview_first` AND `preview_capability==true_preview`). The service's `withResolvedRoutePayload` merges (it only *adds* provider/model/route keys), so `delivery_mode` and `preview_capability` survive into the persisted JSON. Lesson: when a worker needs a resolver-derived attribute, persist it alongside the route at creation time — the carrier is already there (`input_payload`), and adding one more provenance key beats either a forbidden re-resolution or a new DB round-trip.

### Entry 109
- **Trigger**: "Retry of a preview_ready job resumes final generation without duplicating preview" — but the worker's existing retry-safety is a top-of-`Process` switch that short-circuits on `completed`/`failed`, and `preview_ready` is *neither*, so a naive add would either short-circuit it (wrong — final never runs) or fall through and regenerate the preview (wrong — duplicate)
- **Category**: surprise (a new non-terminal status that must *resume mid-lifecycle*, not short-circuit and not restart)
- **Note**: The Phase 7A terminal switch handles `completed` (commit cost) and `failed` (release cost) and returns. `preview_ready` is deliberately left out of that switch so it *falls through* into `processPreviewFirst` — which is exactly right, because the resume point is inside the two-phase function, not at the top. The actual idempotency guard is `len(job.PreviewAssetIds) > 0`: when set, the whole preview phase (MarkRunning, attempt, `Generate(preview)`, `InsertPreview`, `MarkPreviewReady`) is skipped and the worker goes straight to final. This keys on the *committed* `preview_asset_ids`, not on job status, so it's robust to a retry that finds the job in `running` (preview generated, MarkPreviewReady not yet committed → regenerate, correct) or `preview_ready` (resume final). Cost is reserved at creation and committed once after final, so a resume neither re-reserves nor double-commits. Proven by seeding a `preview_ready` job with a prior `preview_asset_id` and asserting the provider is called exactly once (final only, at 1024px), no second preview row, one commit. Lesson: a resumable intermediate status belongs *outside* the terminal short-circuit and keys its skip-guard on the durable artifact it already committed (`preview_asset_ids`), not on the status label.

### Entry 110
- **Trigger**: Phase 7C-1's hardest requirement — "a cancelled job must not get a final or preview output recorded" — collided with the worker's existing shape, which inserts the asset (its own transaction) and *then* marks the job completed (a second transaction). A cancel landing in that gap would leave an asset attached to a cancelled job.
- **Category**: surprise (a correctness race that a conditional `MarkCompleted` alone cannot close)
- **Note**: The naive fix — make `MarkCompleted`/`MarkPreviewReady` conditional on `status != cancelled` — does not help, because the asset is already inserted by the time the conditional update no-ops. The asset would persist with the job stuck cancelled. The real fix is to fold the asset insert AND the job transition into **one** transaction guarded by a `SELECT ... FOR UPDATE` on the `generation_jobs` row (`InsertFinalAssetAndCompleteJobIfNotCancelled` / `InsertPreviewAssetAndMarkPreviewReadyIfNotCancelled`): lock the row, re-read status, and if `cancelled` insert nothing and return `skipped_cancelled`. Admin cancel takes the **same** row lock and releases the reservation in its own transaction, so cancel and persist serialize on the row — whichever commits first wins, and a cancelled job can never end up with an output asset. The forced-regeneration supersede had to move inside this guarded transaction too (I exposed `assets.SupersedeArtifactSlotWithQueries` so the slot archive + insert run under the same lock). The honest boundary: cancel does **not** abort an in-flight provider HTTP call — provider work may finish, but its result is discarded at the guarded persist. I documented that limitation in the OpenAPI path, the confidence index, and here rather than pretend cancel is preemptive. Lesson: "X must never be recorded for a cancelled job" is an atomicity requirement, not a status-check requirement — the asset write and the status flip must be one transaction under a row lock that the canceller also takes, or the gap between them is exploitable.

### Entry 111
- **Trigger**: "Retry must re-reserve cost on the persisted provider/model and must NOT re-resolve the route" — but `cost.ReserveInput` needs `operation_type` and `units`, and `generation_jobs` persisted only `provider_id`/`model_id`/`provider_route_id` on the payload, not the operation or unit count.
- **Category**: process (re-reservation needs priced inputs that weren't persisted)
- **Note**: Phase 7A persisted the resolved route (provider/model/route) on `input_payload` so the worker never re-resolves. But pricing also keys on `operation_type` + `units`, which lived only in the create-time `CreateAndEnqueueParams`, not on the job. Re-resolving to recover them is forbidden (it would re-pick a route). So I made the create path additionally stamp `operation_type` and `units` onto `input_payload` (`withCostContextPayload`) — pure provenance alongside the route, harmless to the worker — and had retry read all five fields straight from the payload. The structural proof that retry never re-resolves is that the `adminjobs` package has **no resolver dependency at all**: it cannot re-resolve even by accident. For jobs created before this slice (no `operation_type`/`units` on the payload), retry falls back to `text_to_image`/`1`, which is correct for the platform's only current operation. Lesson: "reuse the resolved decision, don't recompute it" means persisting *every* input the downstream step needs — the route AND its pricing context — so the retry path is a pure read, and proving the negative ("never re-resolves") is easiest when the code structurally can't.

### Entry 112
- **Trigger**: "Budget reset is lazy, at reservation time, inside the reservation transaction" + "two concurrent reservations must not both reset and double-spend" — a reset that zeroes `spent_amount` is exactly the kind of write two racers can apply twice.
- **Category**: surprise (a lazy rollover that must be idempotent under concurrency without a scheduler)
- **Note**: The temptation is to read the budget, compare `now()` to the window end in Go, and conditionally `UPDATE spent_amount=0`. Two racers both read the stale window, both reset, and the second clobbers the first's hold accounting. The fix is a single conditional `UPDATE ... SET period_start = <window>, spent_amount = 0, status = CASE WHEN 'exceeded' THEN 'active' ... WHERE id = $1 AND period_start < <window>` run inside the same transaction (savepoint) as the reserve. The `WHERE period_start < window` guard plus the row lock the `UPDATE` takes (held until the outer transaction commits) means the first racer advances `period_start`, and the second's predicate is now false → it no-ops. Crucially the reset touches only `spent_amount` and `status`, **never** `reserved_amount`, so a live hold opened just before the reset survives until its job terminates (the edge case the spec calls out). Computing the window in SQL (`date_trunc(... AT TIME ZONE 'UTC')`) rather than injecting a clock kept it atomic and let tests force a boundary by writing `period_start` into the past directly. Lesson: a lazy, idempotent rollover is a single guarded `UPDATE` whose `WHERE` encodes "only if not already rolled" — not a read-modify-write — and it must leave in-flight holds alone so the rollover never erases money the platform is still tracking.

### Entry 113
- **Trigger**: Phase 7C-2's hardest requirement — "the concurrent-job cap is hard-enforced under parallel requests" AND "an idempotency replay must never be denied by the cap, even at the cap" — two requirements that pull in opposite directions: the cap wants to count-and-reject early, replay wants to short-circuit even earlier, and both have to survive N same-token requests racing through one transaction.
- **Category**: surprise (an ordering constraint where the "obvious" placement of the cap check is wrong in two distinct ways)
- **Note**: The naive design counts live jobs first and rejects at the cap. That breaks idempotency: a legitimate retry of an existing job (same Idempotency-Key) would be counted against the cap and 429'd even though it creates no new load. The fix is a strict in-transaction order, all under a per-token transaction-scoped advisory lock (reusing the Phase 6A4 `AcquireSupersedeLock`/`pg_advisory_xact_lock` helper with a `concurrent:<token_id>` key): (1) take the lock so same-token creates serialize; (2) **if an idempotency key exists, look it up under the lock and replay before counting** — this both honors "replay wins" and closes the concurrent same-key duplicate race (a second same-key request blocks on the lock until the first commits, then sees the committed row and replays instead of being denied); (3) only then count live jobs and reject at the cap. The handler pre-check (`LookupReplay`) is the *other* replay point and runs before `CreateAndEnqueue` is even called, so a recorded key never reaches the cap at all. The advisory lock is what makes the cap *hard*: without it, N parallel creates each read `count < cap` before any inserts, and all N slip through. With it they serialize, so the (cap+1)th sees the committed predecessors and is rejected. Proven by fanning out 10 concurrent creates at cap 3 and asserting exactly 3 succeed + exactly 3 live jobs, and separately fanning out 8 same-key creates at cap 1 and asserting all 8 return the *same* job with zero denials. Lesson: when a throttle and an idempotency guard share a code path, the guard must be evaluated *first and under the same lock that makes the throttle hard* — otherwise you either double-count replays or let parallel requests race past the cap, and you cannot fix both by reordering alone.

### Entry 114
- **Trigger**: "Redis-backed rate limiting" + "the counter increment and TTL creation must be atomic" + "a connection drop between INCR and EXPIRE must not leave a permanent key" — a correctness requirement that the obvious two-call `INCR` then `EXPIRE` cannot satisfy.
- **Category**: process (a durability hazard hiding in a two-statement sequence that looks atomic but isn't)
- **Note**: `INCR key` followed by `EXPIRE key ttl` is two round-trips. If the connection drops (or the process dies) between them, the key exists with **no TTL** and never expires — the token's window is wedged shut forever, and request-rate limiting silently turns into a permanent ban. The fix is a single server-side Lua script (`INCR`; `if count == 1 then PEXPIRE end; return count`) so the first increment that *creates* the key also sets its expiry in one atomic execution; later increments within the window leave the TTL untouched (true fixed window). Kept the backing store behind a tiny `Store{ Increment(ctx, key, ttl) }` interface so unit tests use an in-memory fake (asserting the TTL is set exactly once on the first increment) and only the integration test touches real Redis. The companion decision was fail-open: a Redis error returns the error up to the middleware, which logs and *allows* the request with headers omitted — because a rate limiter that takes down the whole API when its datastore blips is worse than one that briefly stops throttling. The concurrent-job cap is Postgres-backed precisely so it keeps holding during that Redis blip. Lesson: "increment a counter with a TTL" is one atomic operation, not two — push it server-side (Lua) so a mid-sequence failure can't strand a TTL-less key, and decide fail-open vs fail-closed *per control* (request-rate fails open; the Postgres cap stays closed).

## 2026-06-14 — Phase 7C-3 (RLS / tenant isolation hardening)

### Entry 115
- **Trigger**: "Once `FORCE ROW LEVEL SECURITY` is enabled, the table owner is also subject to policies" + "the existing integration harness will fight RLS if this is left implicit" — the app and the whole prior integration suite have always connected as the table-owning role, which normally bypasses RLS.
- **Category**: process (a hardening layer that, done naively, silently breaks every existing query OR silently proves nothing)
- **Note**: RLS has a bimodal failure here. If the app/tests keep connecting as a role that bypasses RLS (owner-without-FORCE, or superuser), every existing test passes — but the new "isolation works" assertions pass *for the wrong reason* and prove nothing, because the role never hit a policy. If instead you flip the default test pool to a non-superuser RLS-enforced role, every prior fixture seed/cleanup explodes: the `api_tokens` INSERT fails `WITH CHECK` (GUC unset → NULL), and tenant-wide cleanup DELETEs match zero rows. The resolution is a deliberate **two-pool split**: a system/BYPASSRLS pool for all fixture setup + every pre-existing test (which keep relying on app-level predicates and pass unchanged), and a separate non-superuser API-role pool used *only* by the new enforcement tests. CI provisions the API role and runs the RLS tests under it; the rest of the suite stays on the owner DSN. Lesson: when you add a DB-enforced safety layer on top of an app that has always trusted itself, the test that "passes" is the dangerous one — pick the connecting role per test on purpose, and prove enforcement under the role that is actually subject to the policy.

### Entry 116
- **Trigger**: "`cost.Lifecycle` (Commit/Release) is dual-context ... It must be executor-agnostic: operate on the tx/pool it is handed, and must NOT choose its own pool or hardcode the system executor" — the same finalizer is called from the worker (system/bypass) and from request-path admin cancel/retry (tenant-local).
- **Category**: design (a shared component whose correctness depends on *whose* transaction it runs in)
- **Note**: The tempting "fix" for an admin-cancel RLS failure is to give the Lifecycle the system pool so its release always bypasses RLS. That defeats the entire PR — the admin path is a legitimate tenant-local operation and should run under the tenant's GUC, not bypass it. The discipline that fell out: the Lifecycle exposes `Commit/Release` (open a tx on the pool it was *handed* — the worker hands it the system pool) **and** `CommitInTx/ReleaseInTx` (operate purely on the caller's tx). The worker uses the standalone form; admin cancel/retry sets `app.current_tenant` inside its own tx and composes the in-tx form. Same code, two executors, zero pool-picking inside the component. The matching rule for the create path: an enqueue-failure that needs to release a reservation post-commit does it inside a fresh `WithTenant` tx (tenant GUC set, `ReleaseInTx` composed in) — **not** by reaching for the system executor. Lesson: a dual-context DB helper must never decide its own executor; hand it the tx (or the pool) and let the caller own the tenant scoping, or you will "fix" an RLS failure by quietly punching a hole through RLS.

### Entry 117
- **Trigger**: "Use `tenant_id = NULLIF(current_setting('app.current_tenant', true), '')` ... Do not cast to UUID. This repo's `tenant_id` is text." + "deny by default" — getting the policy expression subtly wrong fails open or fails at runtime.
- **Category**: knowledge (one SQL expression carries three separate correctness properties)
- **Note**: The policy predicate is doing three things at once, and each is load-bearing. (1) **Text, not uuid** — ids look like `tenant_it_jobs`; a `::uuid` cast in the policy would raise on every runtime query, not just leak. (2) **`current_setting(..., true)`** — the second arg `true` (missing_ok) makes an unset GUC return `NULL` instead of erroring, which is what *lets* deny-by-default work instead of throwing. (3) **`NULLIF(..., '')`** — an empty-string GUC (e.g. a `set_config` with `''`) must also collapse to `NULL`, otherwise `tenant_id = ''` would match nothing-or-something depending on data and you'd have an inconsistent gate. Together they guarantee `unset OR empty ⇒ NULL ⇒ zero rows`. I validated all of it against a real Postgres under the non-superuser role before trusting it: tenant-A-vs-B visibility, unset-GUC-sees-zero, `WITH CHECK` rejects a cross-tenant insert, child tables (`idempotency_keys` via `api_tokens`) isolate through a parent-join `EXISTS`, and global reference tables stay readable with no GUC. Lesson: an RLS policy is a security control written as one expression — read every token of it as a requirement, and prove it under the role that is actually subject to it, because a policy that "looks right" can fail open (wrong NULL handling) or fail closed-at-runtime (uuid cast) in ways no superuser test will ever reveal.
